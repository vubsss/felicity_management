...
const OrganizerEventForm = () => {
	...
	const handleSubmit = async (event) => {
		...
		if (endTime && registrationDeadline && registrationDeadline >= endTime) {
			setError('Registration deadline must be before end time.')
			return
		}
				...
				<div className="flex flex-wrap items-center justify-between gap-3">
					...
					{isEditing && (
						<div className="flex flex-wrap gap-2">
								...
								<>
									{registrationStatus === 'open' ? (
										<button
											...
											type="button"
											onClick={() => handleStatusChange({ registrationStatus: 'closed' })}
										>
											Close registrations
										</button>
									) : (
										<button
											className="btn btn-outline"
											type="button"
											onClick={() => handleStatusChange({ registrationStatus: 'open' })}
										>
											Open registrations
										</button>
									)}
							...
							{status === 'published' && (
								<button className="btn btn-outline" type="button" onClick={() => handleStatusChange({ status: 'closed' })}>
									Close event
								</button>
							)}
							{status === 'ongoing' && (
								<>
									<button className="btn btn-outline" type="button" onClick={() => handleStatusChange({ status: 'completed' })}>
										Mark completed
									</button>
									<button className="btn btn-outline" type="button" onClick={() => handleStatusChange({ status: 'closed' })}>
										Close event
															...
															<div key={variantIndex} className="flex gap-2 items-end">
																...
																<button
																	...
																	onClick={() =>
																		setForm((prev) => {
																			const newItems = [...(prev.merchandise?.items || [])]
																			const newVariants = newItems[itemIndex].variants.filter((_, idx) => idx !== variantIndex)
																			newItems[itemIndex] = { ...newItems[itemIndex], variants: newVariants }
																			return { ...prev, merchandise: { ...prev.merchandise, items: newItems } }
																		})
																	}
